...
export default function MentorAbsenceApprovalScreen() {
  ...
  const loadRequests = async () => {
    try {
      ...
      // Filter for requests that need mentor approval
      const pendingForMentor = response.data.requests.filter(
        (r: AbsenceRequest) => r.mentorDecision === 'PENDING'
      );
      setRequests(pendingForMentor);
    } catch (error) {
          ...
          requests.map((request) => (
            <View key={request.id} style={styles.requestCard}>
              <View style={styles.requestHeader}>
                ...
                <View style={styles.parentDecision}>
                  <Text style={styles.parentLabel}>Parent:</Text>
                  <Text
                    style={[
                      styles.parentStatus,
                      request.parentDecision === 'APPROVED' && { color: '#10B981' },
                      request.parentDecision === 'REJECTED' && { color: '#EF4444' },
                    ]}
                  >
                    {request.parentDecision}
                  </Text>
                </View>
              </View>
              ...
              {request.parentComment && (
                <View style={styles.parentComment}>
                  <Text style={styles.commentLabel}>Parent's comment:</Text>
